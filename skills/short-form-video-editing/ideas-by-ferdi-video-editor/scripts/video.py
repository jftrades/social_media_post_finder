...
def gate(job):
    ...
    required+=('original_audio','visual_timing','retiming','broll_fallback','flash_shutter_intro') if mode=='voiceover' else ('cleanup',)
...
def plan(job, work):
    ...
    for i, source in enumerate(job['sources']):
        ...
        intervals = []
        if job['cleanup']:
            tight = job.get('tight_cleanup') is True
            pre_roll, post_roll, join_gap = ((.03, .05, .03) if tight else (.06, .10, .29))
            for w in kept:
                start, end = max(0, w['start']-pre_roll), min(length, w['end']+post_roll)
                if intervals and start-intervals[-1][1] <= join_gap and not any(d['start'] < start and d['end'] > intervals[-1][1] for d in drops):
...
FLASH_SHUTTER_FRAME_COUNTS = (8,9,8,9,8,8,9)
FLASH_SHUTTER_CUT_FRAMES = tuple(sum(FLASH_SHUTTER_FRAME_COUNTS[:i]) for i in range(1,7))
FLASH_SHUTTER_TIMES = tuple((frame-6)/30 for frame in FLASH_SHUTTER_CUT_FRAMES)
FLASH_SHUTTER_DURATION = sum(FLASH_SHUTTER_FRAME_COUNTS)/30


def flash_shutter_gate(job):
    enabled=job.get('flash_shutter_intro_enabled',False)
    clips=job.get('flash_shutter_intro_clips',[])
    if job.get('mode') != 'voiceover':
        if enabled or clips:
            raise ValueError('FlashShutterIntro is available only for voice-over projects.')
        return
    answer=job.get('intake',{}).get('flash_shutter_intro')
    if answer not in ('ja','nein') or enabled is not (answer=='ja'):
        raise ValueError('Ask Voice-over Level 3: FlashShutterIntro ja/nein.')
    if not enabled:
        if clips:
            raise ValueError('FlashShutterIntro clips must be empty when disabled.')
        return
    if not isinstance(clips,list) or len(clips)!=7 or len(set(clips))!=7 or not all(isinstance(x,str) and x.strip() for x in clips):
        raise ValueError('FlashShutterIntro requires exactly seven distinct project clips in story order.')
    project_root=f"project_videos/short_form_unfinished_projects/{job.get('project','')}/".casefold()
    if any(not p.replace('\\','/').casefold().startswith(project_root) for p in clips):
        raise ValueError('FlashShutterIntro may use only clips from the confirmed unfinished project folder, not library B-roll.')


def add_flash_shutter_intro(job,work,base,length):
    if not job.get('flash_shutter_intro_enabled'):
        return base
    flash_shutter_gate(job)
    specs={v['path']:v for v in job.get('visuals',[]) if isinstance(v,dict) and 'path' in v}
    parts=[];report=[]
    for index,(path,frames) in enumerate(zip(job['flash_shutter_intro_clips'],FLASH_SHUTTER_FRAME_COUNTS)):
        source=media(path);source_duration=duration(source);clip_duration=frames/30
        start=max(0,(source_duration-clip_duration)/2)
        spec=specs.get(path,{})
        part=f'flash-source-{index}.mkv'
        ff(['-ss',start,'-i',source,'-an','-filter_complex',
            video_filter(spec.get('capture',job['capture']),spec.get('rotate',job.get('camera_rotate',0) if spec.get('capture',job['capture']) == 'camera' else 0),
                         spec.get('camera_lut_rec709_mix',job.get('camera_lut_rec709_mix',.75)),spec.get('camera_lut_look_mix',job.get('camera_lut_look_mix',.30))),
            '-map','[v]','-frames:v',frames,'-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p',part],work)
        parts.append(part)
        report.append({'path':path,'source_duration':source_duration,'start':start,'end':start+clip_duration,'frames':frames})
    args=[];filters=[]
    for index,(part,frames) in enumerate(zip(parts,FLASH_SHUTTER_FRAME_COUNTS)):
        args += ['-i',part]
        filters.append(f'[{index}:v]trim=end_frame={frames},setpts=PTS-STARTPTS[s{index}]')
    filters.append(''.join(f'[s{i}]' for i in range(7))+'concat=n=7:v=1:a=0[intro]')
    ff(args+['-filter_complex',';'.join(filters),'-map','[intro]','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','flash-intro.mkv'],work)
    ff(['-i','flash-intro.mkv','-i',base,'-filter_complex',
        f'[0:v]setpts=PTS-STARTPTS[i];[1:v]trim=start={FLASH_SHUTTER_DURATION},setpts=PTS-STARTPTS[b];[i][b]concat=n=2:v=1:a=0[v]',
        '-map','[v]','-map','1:a','-t',length,'-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','-c:a','copy','flash-base.mkv'],work)
    effect=media('agent_tooling/sound_fx/kauasilbershlachparodes-shutter-click-2-494026.mp3')
    graph=[f'[1:a]aresample=48000,volume=-6dB,asplit={len(FLASH_SHUTTER_TIMES)}'+''.join(f'[s{i}]' for i in range(len(FLASH_SHUTTER_TIMES)))]
    for index,at in enumerate(FLASH_SHUTTER_TIMES):
        graph.append(f'[s{index}]adelay={round(at*48000)}S:all=1[d{index}]')
    graph.append('[0:a]'+''.join(f'[d{i}]' for i in range(len(FLASH_SHUTTER_TIMES)))+f'amix=inputs={len(FLASH_SHUTTER_TIMES)+1}:duration=first:normalize=0[a]')
    ff(['-f','lavfi','-i',f'anullsrc=r=48000:cl=stereo:d={length}','-i',effect,
        '-filter_complex',';'.join(graph),'-map','[a]','-c:a','pcm_s16le','flash-shutter-sfx.wav'],work)
    save(work/'flash-shutter-report.json',{'name':'FlashShutterIntro','target_snippet_duration':.28,
         'frame_counts':FLASH_SHUTTER_FRAME_COUNTS,'cut_times':[x/30 for x in FLASH_SHUTTER_CUT_FRAMES],
         'shutter_times':FLASH_SHUTTER_TIMES,'lead_seconds':.2,'sound_db':-6,'clips':report})
    return 'flash-base.mkv'
...
def multiline_title_image(job,work):
    ...
    for text,spec in zip(lines,MULTILINE_TITLE_PRESETS[style]):
        if not text:
            ...
            continue
        if spec.get('star') and job.get('title_bottom_star',True) and not text.startswith('*'):
            text='*'+text
...
def transparent_speech_gain(input_lufs, input_true_peak, target_lufs=-16.0, peak_ceiling=-2.0):
    """Return one constant gain value; never compress or reshape the speech."""
    values = (input_lufs, input_true_peak, target_lufs, peak_ceiling)
    if not all(isinstance(x, (int, float)) and math.isfinite(x) for x in values):
        raise ValueError('Speech loudness values must be finite numbers')
    return min(target_lufs-input_lufs, peak_ceiling-input_true_peak)
...
def voiceover_visual_gate(job):
    ...
    for visual in job.get('visuals',[]):
        ...
        if visual.get('original_audio'):
            ...
            continue
        if visual.get('full_clip'):
            if abs(visual['start']) > .02 or abs(visual['end']-source_duration) > .03 or visual.get('speed',1) != 5:
                raise ValueError('A full-clip retime must use the complete source at the confirmed five-times speed.')
            continue
        if visual.get('continuous_key'):
...
def render(job, work):
    ...
    for n, s in enumerate(p['segments']):
        ...
        ff(args + ['-map', '0:a:0', *([] if job['mode']=='voiceover' else ['-map','[v]']), '-c:a', 'pcm_s24le', '-ar','48000','-ac','2',part],work)
    ...
    if job['mode'] == 'voiceover':
        ...
        for n,v in enumerate(visuals):
            ...
            graph=video_filter(v.get('capture',job['capture']),v.get('rotate',job.get('camera_rotate',0) if v.get('capture',job['capture']) == 'camera' else 0),v.get('camera_lut_rec709_mix',job.get('camera_lut_rec709_mix',.75)),v.get('camera_lut_look_mix',job.get('camera_lut_look_mix',.30)))
            graph+=f";[v]setpts=(PTS-STARTPTS)/{v.get('speed',1)},fps=30[retimed]"
            # Limit source time before decoding; output-side -t would let retimed
            # clips run for the unretimed duration and shift every later visual.
            ff(['-ss',v['start'],'-t',v['end']-v['start'],'-i',media(v['path']),'-an','-filter_complex',graph,'-map','[retimed]','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p',name],work)
    ...
    base = add_flash_shutter_intro(job,work,base,p['duration'])
    if job.get('title_behind_person'):
        run(['uv','run',Path(__file__).with_name('cutout.py'),work/base,work/'plan.json'])
    # Measure once, then use only constant gain. This preserves dynamics and tone.
    m = {'enabled':False}
    norm = 'anull'
    if job.get('audio_normalize',True):
        ...
        if not all(math.isfinite(float(raw[k])) for k in ('input_i','input_tp')):
            raise ValueError('Speech is silent or cannot be normalized')
        gain = transparent_speech_gain(float(raw['input_i']),float(raw['input_tp']))
        m = {**raw,'enabled':True,'mode':'constant_gain','applied_gain_db':gain,
             'target_lufs':-16.0,'peak_ceiling_dbtp':-2.0}
        norm = f'volume={gain:.6f}dB'
    ff(['-i',base,'-vn','-af',f'apad=pad_dur=3,{norm},atrim=duration={p["duration"]}', '-c:a','pcm_s24le','-ar','48000','speech-normalized.wav'],work)
    ...
    if job.get('music'):
        ...
        graph += f"[{next_input}:a]volume={float(job.get('music_db',-20))}dB[music];"
        mix += '[music]'
        count += 1
        next_input += 1
    if events:
        ...
        count += 1
        next_input += 1
    if job.get('flash_shutter_intro_enabled'):
        args += ['-i','flash-shutter-sfx.wav']
        mix += f'[{next_input}:a]'
        count += 1
    ...
    log = ff(args+['-c:v','libx264','-crf','18','-preset','fast','-pix_fmt','yuv420p','-c:a','aac','-b:a','256k','-ar','48000','-movflags','+faststart','-t',p['duration'],'final.mp4'],work)
...
def finish(job, work):
    ...
    dest = PROJECTS/'short_form_finished_projects'/job['project']
    ...
    input_root = (PROJECTS/'short_form_unfinished_projects').resolve()
